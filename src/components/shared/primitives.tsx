...
export const btn = (variant = "primary"): CSSProperties => ({
  ...
  ...(variant === "primary"   ? { background: "var(--brand)", color: "#ffffff" } :
      variant === "secondary" ? { background: "var(--input-bg)", color: "var(--ink)", border: "1px solid var(--input-border)" } :
      variant === "ghost"     ? { background: "none", color: "var(--brand)" } :
...
export const Toast = ({ msg, type, onClose }: { msg: string; type: string; onClose: () => void }) => {
  ...
  const colors: Record<string, string> = { success: "#10B981", error: "#DC2626", info: "var(--brand)", warning: "#C2410C" };
  return (
...
export const Field = ({ label, children, required }: { label: string; children: ReactNode; required?: boolean }) => (
  <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
    <label style={{ fontSize: 12, fontWeight: 600, color: "var(--muted)", letterSpacing: "0.5px", textTransform: "uppercase" }}>{label}{required && <span style={{ color: "var(--brand)" }}> *</span>}</label>
    {children}
  </div>
);
